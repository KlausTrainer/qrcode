...
-spec encode(binary()) -> tuple().
encode(Bin) ->
	encode(Bin, 'M').

-spec encode(binary(), ec_level()) -> tuple().
encode(Bin, ECLevel) when is_binary(Bin) ->
	Params = choose_qr_params(Bin, ECLevel),
	...
	#qrcode{version = Version, ecc = ECLevel,
            dimension = Dim + ?QUIET_ZONE * 2, data = QRCode}.
...
%%
choose_version(Type, ECLevel, Length) ->
	choose_version(Type, ECLevel, Length, ?TABLES).
%
choose_version(byte, ECLevel, Length, [{{ECLevel, Version}, {_, _, Capacity, _}, ECCBlocks, Remainder}|_])
		when Capacity >= Length ->
	{byte, Version, ECCBlocks, Remainder};
choose_version(Type, ECLevel, Length, [_|T]) ->
	choose_version(Type, ECLevel, Length, T).
